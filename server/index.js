...
import { deployToNetlify } from './netlifyService.js';
...
app.post('/api/deploy', async (req, res) => { try { if (!req.body?.html) return res.status(400).json({ error: 'Generate a website before deploying.' }); res.json(await deployToNetlify(req.body.html)); } catch (error) { res.status(400).json({ error: error.message }); } });
